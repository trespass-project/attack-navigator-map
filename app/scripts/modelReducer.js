...
function reducer(state=initialState, action) {
	...
	switch (action.type) {
		...
		case constants.ACTION_cloneNode: {
			...
			const newGraph = modelHelpers.cloneNode(state.graph, nodeId, cb);
			return mergeWithState({ graph: newGraph });
		}
		...
		case constants.ACTION_cloneGroup: {
			const { groupId, cb } = action;
			const newGraph = modelHelpers.cloneGroup(state.graph, groupId, cb);
			return mergeWithState({ graph: newGraph });
		}
		...
		case constants.ACTION_updateProcess: {
			const { process } = action;
			const newGraph = modelHelpers.updateComponentProperties(
				state.graph,
				'process',
		...
		case constants.ACTION_removeProcess: {
			const { processId } = action;
			const newGraph = modelHelpers.removeProcess(state.graph, processId);
			return mergeWithState({ graph: newGraph });
		}
		...
		case constants.ACTION_updatePolicy: {
			const { policy } = action;
			const newGraph = modelHelpers.updateComponentProperties(
				state.graph,
				'policy',
